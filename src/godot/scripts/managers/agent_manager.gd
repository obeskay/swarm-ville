...
signal agent_moved(user_id: String, x: float, y: float)
...
func _ready() -> void:
	...
	NetworkManager.user_joined.connect(_on_user_joined)
	NetworkManager.user_left.connect(_on_user_left)
	NetworkManager.position_update.connect(_on_position_update)
