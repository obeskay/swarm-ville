# Spec: State Management (AutoLoad Singletons)

**Status:** Proposed
**Scope:** Global state services via Godot AutoLoad
**Related:** `ui-framework/spec.md`, `agent-system/spec.md`

## Overview

Define the AutoLoad singleton services that replace React state management (Zustand) with Godot's global singleton pattern.
...
1. Create `res://scripts/autoloads/game_config.gd` with all constants
2. Create `res://scripts/autoloads/theme_manager.gd` with color system
3. Create `res://scripts/autoloads/agent_registry.gd` with agent tracking
4. Create `res://scripts/autoloads/space_manager.gd` with space state
5. Create `res://scripts/autoloads/websocket_client.gd` with backend communication
6. Create `res://scripts/autoloads/input_manager.gd` with input handling
7. Register all AutoLoads in Godot project settings (autoload list)
8. Test initialization order: GameConfig → ThemeManager → others
9. Test signal connections work across all AutoLoads
10. Document usage patterns in developer guide

---

**Related specs:** `ui-framework/spec.md`, `rendering-system/spec.md`, `agent-system/spec.md`
